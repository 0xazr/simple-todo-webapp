            ...
            <div class="w-full bg-neutral-700 rounded-sm  px-4 pt-4">
                <div class="flex items-center">
                    <div class="block mt-1">
                        <input @click="Task.toggleTaskDone(id)" type="checkbox" class="w-7 h-7 accent-neutral-300 bg-neutral-700 rounded-full text-neutral-800 focus:ring-0" :checked="Task.detail_task.data.is_done">
                    </div>
                    <div class="w-full ml-1 mr-8">
                        <input @keyup.enter="Task.updateTaskTitle(id)" type="text" v-model="Task.detail_task.data.title" class="w-full bg-neutral-700 border-0 focus:ring-0" :class="{'line-through': Task.detail_task.data.is_done}">
                    </div>
                ...
                <div v-for="(step, index) in Task.detail_task.data.steps" :key="index">
                    <div class="flex items-center">
                        ...
                        <div class="mx-1 border-b-[1px] w-full mr-7">
                            <input @keyup.enter="Task.updateStepTitle(id, index, step.title)" type="text" v-model="step.title" class="w-full bg-neutral-700 border-0 focus:ring-0 text-sm" :class="{'line-through': step.is_done}">
                        </div>
